...
| 功能       | 用途                                                      |
| ---------- | --------------------------------------------------------- |
| 单段配音   | 选择声音与情绪，先试听 30 字，再生成并导出 MP3            |
| 字幕配音   | 导入 SRT/TXT，按句编辑、试听、逐句缓存并合成完整音轨      |
| 多人对话   | 直接拆分规整脚本，或用 API 从小说和分镜稿中提取角色台词   |
| 我的声音   | 一个声音可保存多段参考录音，并随时切换当前使用的样音      |
| 项目与记录 | 保存稿件、模型、声音和参数；按日期试听、收藏、导出或删除  |
| 本地引擎   | 下载、暂停、续传、换源、离线导入和迁移模型                |
| 智能文稿   | 用自己的文本 API 整理口语、停顿、发音和翻译，确认后再替换 |
...
3. 回到“开始创作”，选择声音、模型、语言和情绪；输入文字后可先“试听 30 字”，满意再生成。
...
在“设置 → API配置”中填写 OpenAI 兼容接口的 Base URL、Model 和 API Key。配置后，“开始创作”可处理口语、停顿、精简、发音、翻译和自定义要求；“多人对话”可把小说、分镜稿或混杂脚本整理成角色台词，并先让你确认再替换。

有选中文字时只处理选区，否则处理全文。接口返回后先显示原稿与修改稿，确认后才替换，并可撤销；发音建议可直接加入项目发音词典。密钥使用 Windows 安全存储加密，界面不会回显。此功能完全可选，未配置或接口不可用时不影响本地克隆、配音和导出。
...
| ----------------------- | -----: | ----------------------------------------- | -------: | ------------------------------ |
| **VoxCPM2**             |  ★★★★★ | 综合最推荐；真实克隆、自由表达、30 种语言 |     8GB+ | 大多数口播、旁白和多语言创作   |
...
- 只有主动使用“智能处理”或“智能整理脚本”时，对应文字才会发送到用户自己配置的文本 API；不会发送录音、音频、模型文件或本地路径。
