...
- Worker 安全测试通过：用系统 Python 启动真实 Worker，使用临时空目录、无需模型权重；仅环回访问、拒绝 Origin、启动 token 只可使用一次、会话 token 鉴权有效。Windows CI 会在 20 秒截止时间内等待真实监听就绪，失败时同时记录探测次数、进程状态和 stdout/stderr；等待策略不会跳过或放宽任何鉴权断言。
